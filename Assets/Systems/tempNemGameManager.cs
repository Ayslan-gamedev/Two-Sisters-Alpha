...
public class tempNemGameManager : MonoBehaviour {
    ...
    private void Update() {
        if(pause != false) {
            ...
            if(inMovment == false && Input.GetKeyDown(KeyCode.P))
                ChangeSister();
        }
    ...
    protected private void MovementSister() {
        ...
        if(atualMovDirection == Vector2.zero) {
            ...
            if(speed > maxSpeed[playerSelected])
                speed = maxSpeed[playerSelected];
            else {
                if(speed < startSpeed[playerSelected])
                    speed = startSpeed[playerSelected];
        ...
        if(Input.GetKeyDown(KeyCode.L))
            atualMovDirection = Vector2.zero;
